    ...
    private void valueFuzz(DateType d) {
        if (!d.hasValue()) {
            ...
            if (fuzzerContext.conditionalChance()) {
                ...
                d.setValue(null);
                fuzzerContext.addLog(new FuzzOperationResult<>("set Value in DataType", value, null));
            } else {
                val value = d.getValue();
                val newValue = fuzzerContext.getRandomDateWithFactor(5);
                d.setValue(newValue);
                fuzzerContext.addLog(new FuzzOperationResult<>("set Value in DataType", value, newValue));
            }
        }
    }
